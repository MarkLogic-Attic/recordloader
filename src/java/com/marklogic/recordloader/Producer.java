...
public class Producer extends InputStream {
		...
		if (copyNamespaceDeclarations) {
			...
			if (stop > 0) {
				...
				for (int i = 0; i < stop; i++) {
					...
					logger.finest("found namespace declaration " + nsDeclPrefix
							+ " = " + nsDeclUri);
					decl
							.append("xmlns"
									+ (nsDeclPrefix == null ? ""
	...
	private boolean processEndElement() throws XmlPullParserException {
		...
		if (!(recordName.equals(name) && recordNamespace.equals(namespace))) {
		...
		if (currentId == null) {
			throw new XmlPullParserException("end of record element " + name
					+ " with no id found: " + Configuration.ID_NAME_KEY + "="
					+ idName);
		}
	 ...
	 * 
	 */
	private void processMalformedEntityRef() throws XmlPullParserException {
		...
		if (config.isUnresolvedEntityIgnore()) {
			...
			String replacement = Configuration.UNRESOLVED_ENTITY_REPLACEMENT_PREFIX
					+ name + Configuration.UNRESOLVED_ENTITY_REPLACEMENT_SUFFIX;
			write(replacement);
			return;
		}
	 ...
	 * 
	 */
	private boolean handleUnresolvedEntity() throws XmlPullParserException,
			IOException {
		int type;
		boolean c = true;
		while (c) {
	...
	private int prepareByteBuffer(int _readSize) throws IOException {
		...
		if (buffer == null) {
			...
			try {
				while (keepGoing && (buffer == null || buffer.length() < _readSize)) {
					processNext();
				}
			} catch (XmlPullParserException e) {
		...
		// logger.finest(byteBuffer == null ? null : new String(byteBuffer));
	...
	public int read() throws IOException {
		...
		int available = prepareByteBuffer(1);

		if (available < 0) {
			//logger.warning("available byte = " + available);
			return available;
		}
	...
	public int read(byte[] b, int off, int len) throws IOException {
		...
		if (available < 0) {
			//logger.warning("available bytes = " + available);
			return available;
		}
		...
		if (copyLen < 1) {
			logger.warning("copyLen = " + copyLen);
		}
		return copyLen;
	}
			...
			case XmlPullParser.END_DOCUMENT:
				throw new XmlPullParserException(
						"end of document before end of current record!\n"
								...
								+ " at " + xpp.getPositionDescription());
			default:
				throw new XmlPullParserException("UNIMPLEMENTED: " + eventType);
			}
		} catch (XmlPullParserException e) {
	 ...
	 * @throws XmlPullParserException
	 */
	public void setSkippingRecord(boolean b) throws XmlPullParserException,
			IOException {
		skippingRecord = b;
	...
	public String getByteBufferDescription() throws IOException {
		int available = prepareByteBuffer(1);
		return "" + byteIndex + "," + available + ": "
				+ (byteBuffer == null ? null : new String(byteBuffer));
	}
